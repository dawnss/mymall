...
import static org.biye.mymall.admin.util.AdminResponseCode.NOTICE_UPDATE_NOT_ALLOWED;
...
public class AdminNoticeController {
    ...
    public Object list(String title, String content,
        ...
        List<MymallNotice> noticeList = noticeService.querySelective(title, content, page, limit, sort, order);
        return ResponseUtil.okList(noticeList);
    }

    private Object validate(MymallNotice notice) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody MymallNotice notice) {
        ...
        // 2. 添加管理员通知记录
        List<MymallAdmin> adminList = adminService.all();
        ...
        for(MymallAdmin admin : adminList){
    ...
    public Object read(@NotNull Integer id) {
        MymallNotice notice = noticeService.findById(id);
        List<MymallNoticeAdmin> noticeAdminList = noticeAdminService.queryByNoticeId(id);
    ...
    @PostMapping("/update")
    public Object update(@RequestBody MymallNotice notice) {
        Object error = validate(notice);
        if (error != null) {
            return error;
        }
        MymallNotice originalNotice = noticeService.findById(notice.getId());
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody MymallNotice notice) {
        // 1. 删除通知管理员记录
