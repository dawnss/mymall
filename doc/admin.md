...
权限控制在数据库层面涉及到三个表`mymall_admin`, `mymall_role`和`mymall_permission`：
* mymall_admin表中存在roleId字段，保存角色ID数组；
* mymall_role表记录角色名称和角色介绍；
* mymall_permission表记录角色所用于的权限值。
...
后端权限基于shiro来实现，相关代码见mymall-admin-api模块。
...
public class AdminAdController {
    ...
    public Object list(String name, String content,
        ...
        List<MymallAd> adList = adService.querySelective(name, content, page, limit, sort, order);
...
mymall-admin模块的代码基于[vue-element-admin](https://github.com/PanJiaChen/vue-element-admin)
