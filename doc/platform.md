...
mymall-db模块提供数据库访问服务。

mymall-core模块提供通用服务。

mymall-all模块则只是一个包裹模块，几乎没有任何代码。该模块的作用是融合两个spring boot模块
和mymall-admin模块静态文件到一个单独Spring Boot可执行jar包中。

mymall-all-war模块和mymall-all模块是一样的作用，只是采用war打包方式。

## 2.2 mymall-db

mymall-db模块是一个普通的Spring Boot应用，基于mybatis框架实现数据库访问操作，对外提供业务数据访问服务。

此外，mymall-db最终是作为一个类库被其他模块所依赖使用，因此并不对外
...
3. 根据`table`, 自动生成三种代码:
   * src文件夹`org.linlinjava.mymall.db.dao` 包内的Java代码
   * src文件夹`org.linlinjava.mymall.db.domain` 包内的Java代码
   * resources文件夹`org.linlinjava.mymall.db.domain.dao` 内的XML文件

以上三种代码即可封装对数据库的操作，开发者无需直接操作sql代码，
而是直接操作Java代码来完成对数据库的访问处理。

关于如何基于mybatis的Example代码来访问数据库，请查阅相关资料，
或者参考本模块`org.linlinjava.mymall.db.service` 包内的Java代码。

当然，为了达到数据库访问效率，开发者也可以手动自定义mapper文件和对应的Java代码。
例如，当需要访问两个表的数据时，这里是在业务层通过Java代码遍历的形式来访问两个表，
也可以通过自定义的mapper文件来实现。

接下来，以`mymall_brand`表举例说明如何自动生成代码：
...
2. 自动生产src文件夹下domain包内的MymallBrand.java类、MymallBrandExample.java类、
    dao包内的MymallBrandMapper.java接口和resources文件夹下dao包内的MymallBrandMapper.xml文件。

3. 手动在service包内创建MymallBrandService.java来对外提供具体的服务。
   ...
   ```java
    ...
    public class MymallBrandService {
       ...
       private MymallBrandMapper brandMapper;
        ...
        public List<MymallBrand> query(int offset, int limit) {
...
1. dao代码
   ...
   在src文件夹`org.linlinjava.mymall.db.domain` 包内的StatMapper.java代码定义了数据库访问的接口

2. domain代码
 
   如果希望数据库操作返回数据模型，可以在src文件夹`org.linlinjava.mymall.db.domain` 包内创建相应代码。
   而这里统计功能是采用简化的`List<Map>`保存数据，没有定义domain代码。

3. XML文件

   在resources文件夹`org.linlinjava.mymall.db.domain.dao` 内的StatMapper.xml文件则是实现真正的数据库访问操作。

4. service代码

   这里可以在`org.linlinjava.mymall.db.service` 内定义一个StatServie.java代码，调用底层mapper代码，对外服务。
    ```
    @Service
...
通常业务层代码在src文件夹`org.linlinjava.mymall.db.service` 包中。
    ...
    ```
        <table tableName="mymall_goods">
            <columnOverride column="gallery" javaType="java.lang.String[]"
                            typeHandler="org.linlinjava.mymall.db.mybatis.JsonStringArrayTypeHandler"/>
        </table>
    ```
3. 使用mybatis generator自动生成代码，可以看到MymallGoods的gallery是`String[]`类型。
...
1. 在数据库里面创建一个表，例如`mymall_demo`:

    ```sql
    CREATE TABLE `mymall`.`mymall_demo` (
      `id` INT NOT NULL AUTO_INCREMENT,
      `name` VARCHAR(45) NULL,
      `address` VARCHAR(45) NULL,
      PRIMARY KEY (`id`));
      
    INSERT INTO `mymall`.`mymall_demo` (`id`, `name`, `address`) 
...
2. 在generatorConfig.xml中增加一个新的table标签
    ...
    ```
    <generatorConfiguration>
         <table tableName="mymall_demo">
             <generatedKey column="id" sqlStatement="MySql" identity="true" />
         </table>
    </generatorConfiguration>
    ```
3. 双击mybatis generator插件，检查MymallDemo.java类、MymallDemoExample.java类、
   MymallDemoMapper.java接口和MymallDemoMapper.xml是否生产。
   
4. 在service里面新建MymallDemoService.java类，

   ```java
    @Service
    public class MymallDemoService {
       @Resource
       private MymallDemoMapper demoMapper;

        public List<MymallDemo> list() {
           MymallDemoExample example = new MymallDemoExample();
           return demoMapper.selectByExample(example);    
        }
    }
   ```

5. 可以在`src/test/java/org.linlinjava.mymall.db`包里面创建MymallDemoTest.java类,
    使用Junit进行测试。
    ...
    @SpringBootTest
    public class MymallDemoTest {    
       @Autowired
       private MymallDemoService demoService;
    
       @Test
       public void test() {    
        List<MymallDemo> mymallDemoList = demoService.list();
        Assert.assertTrue(mymallDemoList.size() != 0);
       }
    
    }
    ```

6. 同样地，可以在Controller中使用MymallDemoService来对外提供服务。
    ...
    public class DemoController {
       ...
       @RequestMapping("/list")
       public Object list(){    
           List<MymallDemo> demoList = demoService.list();   
           return demoList;
       }
    }
    ```
...
mymall-db模块中不涉及到事务管理，而是在其他后台服务模块中涉及。
但是其他后台服务模块因为依赖mymall-db模块，因此这里列出。
...
* system
  ...
  通过mymall-db模块的数据库访问，读取本项目系统配置信息。
...
注意
> 这里的util代码不会涉及具体业务，例如mymall-db模块中存在一个
> OrderUtil类处理数据库中mymall_order表的一些转换工作。
...
查看mymall-all模块，代码仅仅只有一个Application类。

实际的原理是mymall-all模块内的pom.xml文件：

1. 打包方式是`jar`，因此最后会打包可执行jar格式；
2. 对mymall-wx-api模块和mymall-admin-api模块依赖，
   因此打包时会作为依赖库而打包到mymall-all模块的输出中；
3. 使用copy-resources插件，在打包时把mymall-admin模块的dist
   文件夹拷贝到mymall-all模块的static文件夹中；而这个文件夹
   正是Spring Boot应用的默认静态文件路径。
   
   注意：
   > 这个插件只是简单的拷贝操作；因此开发者应该在打包mymall-all
   > 之前确保先编译mymall-admin模块得到最终静态文件。


## 2.5 mymall-all-war

mymall-all-war模块就是对mymall-all模块进行少量调整，
最后打包时会在target目录下面生成mymall.war，用于tomcat部署。