...
mymall是一个简单的商场系统，基于现有的开源项目，重新实现一个完整的前后端项目，包含小程序客户端、移动客户端和网页管理端。
...
* 基础系统子系统(platform)
  ...
  由数据库、mymall-core模块、mymall-db模块和mymall-all模块组成;

* 小商场子系统(wxmall，即weixin mall)

  由mymall-wx-api模块、mymall-wx模块和renard-wx模块组成；

* 轻商城子系统(mobmall，即mobile mall)

  由mymall-wx-api模块和mymall-vue模块组成。
  注意，目前这里移动商城子系统的后端和小商场子系统是一样的。
    
* 管理后台子系统(admin)

  由mymall-admin-api模块和mymall-admin模块组成。

而九个模块的开发设计到三种技术栈：

* Spring Boot技术栈

  采用IDEA开发工具，开发mymall-core、mymall-db、mymall-admin-api、
  mymall-wx-api和mymall-all共五个模块；
  
* miniprogram（微信小程序）技术栈

  采用微信小程序开发工具，开发mymall-wx模块和renard-wx模块；
  
* Vue技术栈

  采用VSC开发工具，开发mymall-admin模块和mymall-vue模块。
...
接下来，从项目的开发、部署（测试）和上线三个阶段介绍mymall。
...
* MySQL数据访问地址`jdbc:mysql://localhost:3306/mymall`
* mymall-wx-api后端服务地址`http://localhost:8080/wx`，数据则来自MySQL
* mymall-admin-api后端服务地址`http://localhost:8080/admin`,数据则来自MySQL
* mymall-admin前端访问地址`http://localhost:9527`, 数据来自mymall-admin-api
* mymall-wx没有前端访问地址，而是直接在微信小程序工具上编译测试开发，最终会部署到微信官方平台（即不需要自己部署web服务器），而数据则来自mymall-wx-api
...
2. 创建数据库、用户权限、数据库表和测试数据;
   数据库文件存放在mymall-db/sql文件夹中，请开发者在MySQL中
   按照顺序运行以下脚本:
   * mymall_schema.sql，用于创建数据库、用户和权限;
   * mymall_table.sql，用于创建表;
   * mymall_data.sql，用于导入测试数据。

注意：
> 建议采用命令行或者MySQL Workbench。如果采用Navicat可能导入失败。

如果开发者运行mymall_schema.sql失败，可以打开该文件：
```
drop database if exists mymall;
drop user if exists 'mymall'@'%';
create database mymall default character set utf8mb4 collate utf8mb4_unicode_ci;
use mymall;
create user 'mymall'@'%' identified by 'mymall123456';
grant all privileges on mymall.* to 'mymall'@'%';
flush privilege
```
...
7. 采用Maven命令编译本项目
   ...
   此时可以看到，mymall-wx-api等模块多了target文件夹，里面是编译出的文件。
   
   或者采用IDEA的Maven插件编译本项目，点击`compile`
   
   如果采用IDEA也可以跳过当前步骤，直接步骤8（因为运行时会自动编译再运行）。
   
8. 采用Maven命令运行本项目的mymall-all

   例如：
   ```
   cd mymall/mymall-all
   mvn spring-boot:run
   ```
   
   如果采用IDEA，则mymall-all模块的Application类
...
> 3. 开发者使用IDEA导入项目或者运行项目时可能会出现**软件卡顿**的现象，这通常是mymall-admin或者mymall-vue的
...
1. 安装微信小程序开发工具；
2. 导入本项目的mymall-wx模块(或者renard-wx模块)文件夹；
3. 编译前，请确定mymall-all模块已经运行，而mymall-wx模块的config文件夹中的api.js已经设置正确的后端数据服务地址；
...
4. 请确定mymall-all模块已经运行，然后点击`登录`，如果能够成功登录，则表明管理后台的前端和后端对接成功，运行正常。

本项目采用VSC（Visual Studio Code）开发mymall-admin模块，开发者也可以采用其他熟悉的IDE。
...
1. 管理后台前端，即mymall-admin模块，配置文件在mymall-admin中，存在三个配置文件`env.development`,`env.deployment`
...
2. 小商场前端，即mymall-wx模块，配置文件是`mymall-wx/project.config.json`和`mymall-wx/api.js`。
...
以及`apis.js`中的`WxApiRoot`，即小商场服务根地址。
    ...
    // 云平台部署时使用
    // var WxApiRoot = 'http://www.abctxyz.com:8080/wx/';
    // 云平台上线时使用
    // var WxApiRoot = 'https://www.menethil.com.cn/wx/';

    ```

3. 管理后台后端和小商城后端，即多个Spring Boot模块，配置文件是每个模块的`mymall-xx/src/main/java/resources`的
...
都是在`application-xx.yml`文件中，而`application.yml`文件仅仅用于引入其他模块的配置文件。
   ...
   例如mymall-all模块的`application.yml`的内容是
   ```
    spring:
        profiles:
        ...
        message:
            encoding: UTF-8
    ```
    因此启动mymall-all模块时，程序首先加载mymall-all的`application.yml`，然后通过`spring.profiles.active`信息
...
如果开发者启动mymall-all模块，则需要配置该模块的`logback-spring.xml`文件
```
...
注意：
> 如果开发者独立启动mymall-wx-api模块，那么则需要配置mymall-wx-api模块的
> 日志配置方式。

#### 1.4.5.2 数据库连接配置

在mymall-db模块的`application-db.yml`文件中配置数据库连接和druid：
...
spring:
  datasource:
    druid:
      url:  jdbc:mysql://localhost:3306/mymall?useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC&allowPublicKeyRetrieval=true&verifyServerCertificate=false&useSSL=false
...
微信登录需要配置两个地方，
首先是小商场前端mymall-wx模块（或renard-wx模块）中`project.config.json`文件的appid
其次是小商场后端mymall-core模块的`application-core.yml`文件：
```bash
mymall:
  wx:
...
注意
> 这里开发者可能会疑惑：小商场后端应该配置在mymall-wx-api模块的`application-wx.yml`文件更合适。
> 这里放置在`application-core.yml`文件中是因为mymall-core模块也依赖小程序appid配置信息。

#### 1.4.5.4 微信支付配置

在mymall-core模块的`application-core.yml`文件中配置微信支付：
```
mymall:
  wx:
    mch-id: 111111
...
mymall-wx-api模块的WxOrderController类的payNotify方法所服务的API地址。

开发阶段可以采用一些技术实现临时外网地址映射本地，开发者可以百度关键字“微信 内网穿透”自行学习。

#### 1.4.5.5 邮件通知配置

在mymall-core模块的`application-core.yml`文件中配置邮件通知服务：
```
mymall:
  notify:
    mail:
...
验证配置成功：
当配置好信息以后，开发者可以运行mymall-core模块的`MailTest`测试类，
独立发送邮件，然后登录邮箱查看邮件是否成功接收。

#### 1.4.5.6 短信通知配置

在mymall-core模块的`application-core.yml`文件中配置短信通知服务：
```
mymall:
  notify:
    # 短消息模版通知配置
...
验证配置成功：
当配置好信息以后，开发者可以mymall-core模块的`SmsTest`测试类中设置手机号和
...
在mymall-core模块的`application-core.yml`文件中配置快递鸟物流服务：
```
mymall:
  notify:
  # 快鸟物流查询配置
...
验证配置成功：
当配置好信息以后，开发者可以mymall-core模块的`ExpressTest`测试类中设置快递公司编码和
...
在mymall-core模块的`application-core.yml`文件中配置对象存储服务：
...
其次，需要明确的是各模块之间的关系：
  ...
  * mymall-wx-api模块会包含mymall-core模块和mymall-db模块，部署在服务器中
  * mymall-admin-api模块会包含mymall-core模块和mymall-db模块，部署在服务器中
  * mymall-all模块则会包装mymall-wx-api模块和mymall-admin-api模块；
  * mymall-wx模块部署在微信开发者工具中，此外数据API地址指向mymall-wx-api所在服务qi地址
  * mymall-admin编译出的静态文件放在web服务器或者tomcat服务器，此外服务器地址设置指向3中mymall-admin-api所在地址
  
最后，**如果项目部署云服务器，则根据开发者的部署环境在以下文件中或代码中修改相应的配置。**

1. MySQL数据库设置合适的用户名和密码信息；
2. 后端服务模块设置合适的配置信息；
3. 小商场前端mymall-wx模块`config/api.js`的`WxApiRoot`设置小商场后端服务的服务地址；
4. 管理后台前端mymall-admin模块`.env.deployment`中的`VUE_APP_BASE_API`设置管理后台后端服务的服务地址。

实际上，最终的部署方案是灵活的：

* 可以是同一云服务器中安装一个Spring Boot服务，同时提供mymall-admin、mymall-admin-api和mymall-wx-api三种服务
* 可以单一云服务器中仅安装一个tomcat/nginx服务器部署mymall-admin静态页面分发服务，
  然后部署两个Spring Boot的后端服务；
* 也可以把mymall-admin静态页面托管第三方cdn，然后开发者部署两个后端服务
...
> 这里很可能是开发者mymall-admin模块的`config/dep.env.js`或者`condig/prod.env.js`
...
上线之前需要修改代码或者配置文件：
1. mymall-wx-api模块的WxOrderController类的payNotify方法的链接换成合适的地址。

   注意：
   > 换成什么地址都可以，但是这里不应该暴露出来。也就是说这个地址是微信商户平台
   > 和这里的小商场后端服务之间的交互API，对外公开会存在安全隐患。
   
2. mymall-core模块需要配置application-core.yml
    ...
    这里的`mymall.wx.notify-url`就是前面开发者自定义的地址。
    
3. mymall-wx模块的`project.config.json`文件调整相应的值，
...
1. MySQL数据库设置合适的用户名和密码信息；
2. 管理后台后端服务模块设置合适的配置信息，建议开发者参考deploy/mymall的外部配置文件，
   这样可以避免开发者对模块内部的开发配置文件造成修改；
3. 管理后台前端mymall-admin模块`.env.production`中的`VUE_APP_BASE_API`设置管理后台后端服务的服务地址。
...
mymall-admin编译得到的前端文件在第一次加载时相当耗时，这里需要一些措施来优化启动速度
...
* deploy/.gitignore
* mymall-admin/.gitignore
* mymall-admin-api/.gitignore
* mymall-core/.gitignore
* mymall-db/.gitignore
* mymall-wx-api/.gitignore
* mymall-all/.gitignore
* .gitignore
...
2. 打包项目时，会编译打包项目相关模块到mymall和db文件夹中；
...
但是存在一个问题，即多模块配置依赖。
例如，mymall-db模块存在数据库配置信息，那么其他模块如何引入
mymall-db模块的配置信息呢？

最简单的方式，就是其他模块把mymall-db模块的配置信息拷贝到自己的
application配置文件中，但是问题就是数据库信息一旦改变则其他模块又要
再次手动修改，非常不方便。

目前本项目采用一种基于`spring.profiles.active`的方式，细节如下：
1. mymall-db模块存在application.yml和application-db.yml两个配置文件，
    在application-db.yml配置文件中存放数据库配置信息；
2. mymall-core模块也存在application.yml和application-core.yml两个配置文件,
    ...
    spring:
        profiles:
            active: core, db
    ```
    因此，如果单独启动mymall-core模块，则会先读取application.yml配置文件，然后基于
    系统会根据`spring.profiles.active`读取application-db.yml和application-core.yml配置文件，
    因此就会自动读取mymall-db模块的配置文件。
3. 以此类推，在mymall-all模块中存在application.yml配置文件，其中内容是
    ```
    spring:
        profiles:
            active:  db, core, admin, wx
    ```
    因此，系统启动mymall-all模块以后，则会先读取application.yml配置文件，然后基于
...
2. 测试或者部署阶段，系统打包成一个mymall.jar二进制jar包，jar包内部配置文件是之前
    开发阶段的配置文件，此时在mymall.jar的同级目录创建相同的配置文件，在这些配置文件则
    保存了测试或者部署阶段的配置信息。启动mymall.jar时，系统会读取当前目录的配置文件，而
    不再读取jar包内部的配置文件。
3. 上线阶段，同样地，在mymall.jar包同级目录创建上线配置文件。
...
* 6xx，管理后台后端业务错误码，具体见mymall-admin-api模块的`AdminResponseCode`类。
* 7xx，小商城后端业务错误码，具体见mymall-wx-api模块的`WxResponseCode`类。
