...
访问受保护商场API采用自定义`X-Mymall-Token`头部
...
3. 请求受保护API则，则采用自定义头部携带此token
        ...
        X-Mymall-Token: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0aGlzIGlzIGxpdGVtYWxsIHRva2VuIiwiYXVkIjoiTUlOSUFQUCIsImlzcyI6IkxJVEVNQUxMIiwiZXhwIjoxNTU3MzM2ODU0LCJ1c2VySWQiOjIsImlhdCI6MTU1NzMyOTY1NH0.JY1-cqOnmi-CVjFohZMqK2iAdAH4O6CKj0Cqd5tMF3M

#### 1.4.3 管理后台自定义Header

访问受保护管理后台API则是自定义`X-Mymall-Admin-Token`头部。
...
3. 请求受保护API时，则采用自定义头部携带此token
        ...
        GET http://localhost:8080/wx/address/list
        X-Mymall-Admin-Token: f2dbcae8-6e25-4f8e-bc58-aa81d512c952
...
然后，采用自定义`X-Mymall-Token`来携带token访问商场API

![](./pics/admin/get_with_token.png)

注意：
> 访问受保护商场API是采用自定义`X-Mymall-Token`头部；
> 而访问受保护管理后台API则是自定义`X-Mymall-Admin-Token`头部。
...
* 在线Swagger文档链接：http://www.abctxyz.com:8080/swagger-ui.html
* 本地Swagger文档链接：http://localhost:8080/swagger-ui.html

此外，也使用了swagger-bootstrap-ui对Swagger进一步增强了使用效果。
* 在线swagger-bootstrap-ui文档链接：http://www.abctxyz.com:8080/doc.html
