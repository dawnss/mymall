package org.biye.mymall.core.qcode;
...
public class QCodeService {
    ...
    public String createGrouponShareImage(String goodName, String goodPicUrl, MymallGroupon groupon) {
        try {
            //创建该商品的二维码
            ...
            //存储分享图
            MymallStorage storageInfo = storageService.store(inputStream2, imageData.length, "image/jpeg",
    ...
    public String createGoodShareImage(String goodId, String goodPicUrl, String goodName) {
        ...
        try {
            ...
            ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
            //存储分享图
            MymallStorage mymallStorage = storageService.store(inputStream2, imageData.length, "image/jpeg",
                    getKeyName(goodId));

            return mymallStorage.getUrl();
        } catch (WxErrorException e) {
            logger.error(e.getMessage(), e);
        } catch (FileNotFoundException e) {
            logger.error(e.getMessage(), e);
