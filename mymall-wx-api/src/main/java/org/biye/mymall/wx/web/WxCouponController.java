...
import org.biye.mymall.wx.vo.CouponVo;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<MymallCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<MymallCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<MymallCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for(MymallCouponUser couponUser : couponList){
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        for (CouponVo cv : couponVoList) {
            MymallCoupon coupon = couponVerifyService.checkCoupon(userId, cv.getCid(), cv.getId(), checkedGoodsPrice, checkedGoodsList);
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        MymallCoupon coupon = couponService.findByCode(code);
        if(coupon == null){
