...
public class WxHomeController {
    ...
    @Autowired
    private MymallAdService adService;
...
//        }
    ...
    private List<Map> getCategoryList() {
        ...
        List<MymallCategory> catL1List = categoryService.queryL1WithoutRecommend(0, SystemConfig.getCatlogListLimit());
        for (MymallCategory catL1 : catL1List) {
            List<MymallCategory> catL2List = categoryService.queryByPid(catL1.getId());
            List<Integer> l2List = new ArrayList<>();
            for (MymallCategory catL2 : catL2List) {
                l2List.add(catL2.getId());
            }

            List<MymallGoods> categoryGoods;
            if (l2List.size() == 0) {
