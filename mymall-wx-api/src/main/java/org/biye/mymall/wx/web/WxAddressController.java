...
import org.biye.mymall.db.service.MymallAddressService;
import org.biye.mymall.db.service.MymallRegionService;
import org.biye.mymall.wx.annotation.LoginUser;
import org.biye.mymall.wx.service.GetRegionService;
...
public class WxAddressController extends GetRegionService {
	...
	public Object list(@LoginUser Integer userId) {
		...
		List<MymallAddress> addressList = addressService.queryByUid(userId);
	...
	public Object detail(@LoginUser Integer userId, @NotNull Integer id) {
		...
		MymallAddress address = addressService.query(userId, id);
		if (address == null) {
	...
	private Object validate(MymallAddress address) {
	 ...
	 */
	@PostMapping("save")
	public Object save(@LoginUser Integer userId, @RequestBody MymallAddress address) {
		if (userId == null) {
			return ResponseUtil.unlogin();
		}
		...
		if (address.getId() == null || address.getId().equals(0)) {
			...
			MymallAddress mymallAddress = addressService.query(userId, address.getId());
			if (mymallAddress == null) {
				return ResponseUtil.badArgumentValue();
			}
	 ...
	 */
	@PostMapping("delete")
	public Object delete(@LoginUser Integer userId, @RequestBody MymallAddress address) {
		if (userId == null) {
			return ResponseUtil.unlogin();
		}
		...
		MymallAddress mymallAddress = addressService.query(userId, id);
		if (mymallAddress == null) {
			return ResponseUtil.badArgumentValue();
		}
