...
import org.biye.mymall.db.service.MymallKeywordService;
import org.biye.mymall.db.service.MymallSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private MymallKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        MymallKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<MymallKeyword> hotKeywordList = keywordsService.queryHots();

        List<MymallSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<MymallKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (MymallKeyword key : keywordsList) {
