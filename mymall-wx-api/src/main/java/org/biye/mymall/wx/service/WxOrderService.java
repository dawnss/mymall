...
import org.biye.mymall.core.express.ExpressService;
import org.biye.mymall.core.express.dao.ExpressInfo;
import org.biye.mymall.core.notify.NotifyService;
import org.biye.mymall.core.notify.NotifyType;
...
import org.biye.mymall.core.util.DateTimeUtil;
...
import org.biye.mymall.db.util.OrderHandleOption;
import org.biye.mymall.db.util.OrderUtil;
import org.biye.mymall.core.util.IpUtil;
import org.biye.mymall.wx.task.OrderUnpaidTask;
...
import static org.biye.mymall.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<MymallOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (MymallOrder o : orderList) {
            ...
            MymallGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<MymallOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            MymallGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                MymallGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        // 收货地址
        MymallAddress checkedAddress = addressService.query(userId, addressId);
        ...
        MymallGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<MymallCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            MymallCart cart = cartService.findById(cartId);
        ...
        for (MymallCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            MymallCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice, checkedGoodsList);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        MymallOrder order = null;
        // 订单
        order = new MymallOrder();
        ...
        // 添加订单商品表项
        for (MymallCart cartGoods : checkedGoodsList) {
            // 订单商品
            MymallOrderGoods orderGoods = new MymallOrderGoods();
        ...
        for (MymallCart checkGoods : checkedGoodsList) {
            ...
            MymallGoodsProduct product = productService.findById(productId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            MymallGroupon groupon = new MymallGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                MymallGroupon baseGroupon = grouponService.queryById(grouponLinkId);
        ...
        if (order.getActualPrice().equals(new BigDecimal("0.00"))) {
            ...
            MymallOrder o = new MymallOrder();
            o.setId(orderId);
            o.setOrderStatus(OrderUtil.STATUS_PAY);
            orderService.updateSelective(o);

            //  支付成功，有团购信息，更新团购信息
            MymallGroupon groupon = grouponService.queryByOrderId(order.getId());
            if (groupon != null) {
                ...
                List<MymallGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
                if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                    for (MymallGroupon grouponActivity : grouponList) {
                        grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                        grouponService.updateById(grouponActivity);
                    }

                    MymallGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        MymallOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        MymallGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            MymallGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
    ...
    public Object goods(Integer userId, Integer ogid) {
        ...
        MymallOrderGoods orderGoods = orderGoodsService.findById(ogid);

        if (orderGoods != null) {
            Integer orderId = orderGoods.getOrderId();
            MymallOrder order = orderService.findById(orderId);
            if (!order.getUserId().equals(userId)) {
                return ResponseUtil.badArgument();
            }
        }
    ...
    public Object comment(Integer userId, String body) {
        ...
        MymallOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        Integer orderId = orderGoods.getOrderId();
        ...
        // 1. 创建评价
        MymallComment comment = new MymallComment();
    ...
    public void releaseCoupon(Integer orderId) {
        List<MymallCouponUser> couponUsers = couponUserService.findByOid(orderId);
        for (MymallCouponUser couponUser: couponUsers) {
            // 优惠券状态设置为可使用
