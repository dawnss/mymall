...
import static org.biye.mymall.wx.util.WxResponseCode.GOODS_NO_STOCK;
import static org.biye.mymall.wx.util.WxResponseCode.GOODS_UNSHELVE;
...
public class WxCartController {
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        List<MymallCart> list = cartService.queryByUid(userId);
        List<MymallCart> cartList = new ArrayList<>();
        // TODO
        // 如果系统检查商品已删除或已下架，则系统自动删除。
        // 更好的效果应该是告知用户商品失效，允许用户点击按钮来清除失效商品。
        for (MymallCart cart : list) {
            MymallGoods goods = goodsService.findById(cart.getGoodsId());
     ...
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody MymallCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断购物车中是否存在此规格商品
        MymallCart existCart = cartService.queryExist(goodsId, productId, userId);
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser Integer userId, @RequestBody MymallCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody MymallCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        Integer productId = cart.getProductId();
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        MymallCart existCart = cartService.findById(userId, id);
        if (existCart == null) {
    ...
    public Object goodscount(@LoginUser Integer userId) {
        ...
        int goodsCount = 0;
        List<MymallCart> cartList = cartService.queryByUid(userId);
        for (MymallCart cart : cartList) {
            goodsCount += cart.getNumber();
        }
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer userCouponId, Integer grouponRulesId) {
        ...
        // 收货地址
        MymallAddress checkedAddress = null;
        ...
        if (checkedAddress == null) {
            ...
            if (checkedAddress == null) {
                checkedAddress = new MymallAddress();
        ...
        if (cartId == null || cartId.equals(0)) {
            ...
            MymallCart cart = cartService.findById(userId, cartId);
            if (cart == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        for (MymallCart cart : checkedGoodsList) {
        ...
        List<MymallCouponUser> couponUserList = couponUserService.queryAll(userId);
        for(MymallCouponUser couponUser : couponUserList){
            MymallCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCouponId(), couponUser.getId(), checkedGoodsPrice, checkedGoodsList);
            if(coupon == null){
                continue;
            }
